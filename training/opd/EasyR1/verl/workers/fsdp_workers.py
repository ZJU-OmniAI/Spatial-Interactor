...
from ..trainer.opd import build_privileged_scoring_batch, compute_topk_fkl
...
class FSDPWorker(Worker):
    ...
    def _process_multi_modal_inputs(self, data: DataProto):
        ...
        if "multi_modal_inputs" not in self._cache:
            ...
            video_nframes = data.meta_info.get("video_nframes")
            ...
            is_qwen3_vl = "Qwen3VLProcessor" in self.processor.__class__.__name__
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_opd_targets(self, data: DataProto):
        """Build detached teacher top-k targets on student-generated prefixes."""
        assert self._has_actor

        self._process_multi_modal_inputs(data)
        privileged_data = build_privileged_scoring_batch(data)
        if "multi_modal_inputs" in data.non_tensor_batch:
            privileged_data.non_tensor_batch["multi_modal_inputs"] = data.non_tensor_batch[
                "multi_modal_inputs"
            ]

        data = data.to(torch.cuda.current_device())
        privileged_data = privileged_data.to(torch.cuda.current_device())
        if self._use_param_offload:
            load_fsdp_model(self.fsdp_module)

        rollout_temperature = self.config.rollout.temperature
        distill_temperature = float(data.meta_info["opd_temperature"])
        top_k = int(data.meta_info["opd_top_k"])
        token_clip = data.meta_info.get("opd_token_kl_clip")
        special_token_ids = (
            list(self.tokenizer.all_special_ids)
            if data.meta_info.get("opd_mask_special_tokens", True)
            else []
        )
        data.meta_info["temperature"] = rollout_temperature
        privileged_data.meta_info["temperature"] = rollout_temperature

        with self.ulysses_sharding_manager:
            privileged_sharded = self.ulysses_sharding_manager.preprocess_data(privileged_data)
            privileged_log_probs, teacher_topk_ids, teacher_topk_log_probs = self.actor.compute_teacher_topk(
                data=privileged_sharded,
                top_k=top_k,
                distill_temperature=distill_temperature,
                special_token_ids=special_token_ids,
            )
            privileged_output = DataProto.from_dict(
                tensors={
                    "privileged_log_probs": privileged_log_probs,
                    "opd_teacher_topk_ids": teacher_topk_ids,
                    "opd_teacher_topk_log_probs": teacher_topk_log_probs,
                }
            )
            privileged_output = self.ulysses_sharding_manager.postprocess_data(privileged_output)

        plain_tensors = {key: data.batch[key] for key in data.batch.keys()}
        plain_tensors["opd_teacher_topk_ids"] = privileged_output.batch["opd_teacher_topk_ids"]
        plain_tensors["opd_teacher_topk_log_probs"] = privileged_output.batch[
            "opd_teacher_topk_log_probs"
        ]
        data = DataProto.from_dict(
            tensors=plain_tensors,
            non_tensors=dict(data.non_tensor_batch),
            meta_info=dict(data.meta_info),
        )

        with self.ulysses_sharding_manager:
            plain_sharded = self.ulysses_sharding_manager.preprocess_data(data)
            plain_log_probs, student_topk_log_probs = self.actor.compute_log_prob_on_support(
                data=plain_sharded,
                distill_temperature=distill_temperature,
            )
            teacher_fkl = compute_topk_fkl(
                teacher_log_probs=plain_sharded.batch["opd_teacher_topk_log_probs"],
                student_log_probs=student_topk_log_probs,
                token_clip=token_clip,
            )
            plain_output = DataProto.from_dict(
                tensors={
                    "old_log_probs": plain_log_probs,
                    "opd_teacher_fkl": teacher_fkl,
                },
                meta_info={"temperature": rollout_temperature},
            )
            plain_output = self.ulysses_sharding_manager.postprocess_data(plain_output)
        ...
        output = plain_output.union(privileged_output).to("cpu")
        return output
