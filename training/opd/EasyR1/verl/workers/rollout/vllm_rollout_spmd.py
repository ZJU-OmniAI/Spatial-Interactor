...
from transformers import AutoTokenizer, PreTrainedTokenizer, PreTrainedTokenizerBase, ProcessorMixin
...
def _enable_fixed_tokenizer_regex() -> None:
    """Propagate the tokenizer regex fix into vLLM's lazy processor loads."""
    if getattr(AutoTokenizer, "_spatial_interactor_fix_mistral_regex", False):
        return
    ...
    AutoTokenizer._spatial_interactor_fix_mistral_regex = True

    original_patch = PreTrainedTokenizerBase._patch_mistral_regex

    def patch_mistral_regex(cls, tokenizer, pretrained_model_name_or_path, *args, **kwargs):
        if kwargs.get("fix_mistral_regex") is None:
            kwargs["fix_mistral_regex"] = True
        return original_patch(tokenizer, pretrained_model_name_or_path, *args, **kwargs)

    PreTrainedTokenizerBase._patch_mistral_regex = classmethod(patch_mistral_regex)
...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        **kwargs,
        ...
        engine_kwargs = {}
        kv_cache_memory_bytes = os.getenv("SPATIAL_INTERACTOR_VLLM_KV_CACHE_MEMORY_BYTES")
        ...
        VLLMHijack.hijack()
        _enable_fixed_tokenizer_regex()

        self.inference_engine = LLM(
            model=model_path,
            tokenizer=os.getenv("SPATIAL_INTERACTOR_VLLM_TOKENIZER_PATH") or None,
                ...
                vllm_inputs.append(
                    {
                        ...
                        "multi_modal_data": _process_multi_modal_data(
                            ...
                            prompts.meta_info.get("video_max_frames"),
