...
from .opd import opd_weight_at_step, prepare_opd_batch
from .rollout_audit import RolloutAuditLogger
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if config.algorithm.opd.enabled:
            if config.algorithm.adv_estimator != AdvantageEstimator.GRPO:
                raise ValueError("OPD privileged-context distillation requires adv_estimator=grpo.")
            if not config.data.privileged_prompt_key:
                raise ValueError("OPD requires data.privileged_prompt_key to be configured.")
            if config.algorithm.opd.loss_weight < 0:
                raise ValueError("OPD loss_weight must be non-negative.")
            if config.algorithm.opd.top_k <= 1:
                raise ValueError("OPD top_k must be greater than 1.")
            if config.algorithm.opd.temperature <= 0:
                raise ValueError("OPD temperature must be positive.")
            if config.worker.actor.ulysses_size != 1:
                raise ValueError("Top-k OPD currently requires worker.actor.ulysses_size=1.")
    ...
    def _validate(self) -> dict[str, Any]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            test_gen_batch.meta_info["video_nframes"] = self.config.data.video_nframes
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            meta_info = {
                ...
                "video_nframes": self.config.data.video_nframes,
            ...
            gen_batch = new_batch.pop(
                batch_keys=["input_ids", "attention_mask", "position_ids"],
                non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data"],
                meta_info_keys=["min_pixels", "max_pixels", "video_fps", "video_nframes", "video_max_frames"],
            )
    ...
    def fit(self):
        ...
        audit_config = self.config.trainer.rollout_audit
        if audit_config.enabled:
            audit_dir = audit_config.output_dir or os.path.join(
                self.config.trainer.save_checkpoint_path, "rollout_audit"
            )
            self.rollout_auditor = RolloutAuditLogger(
                output_dir=audit_dir,
                tokenizer=self.tokenizer,
                log_freq=audit_config.log_freq,
                groups_per_step=audit_config.groups_per_step,
                expected_group_size=self.config.worker.rollout.n,
                assistant_prefill=self.config.data.assistant_prefill,
                include_privileged_context=audit_config.include_privileged_context,
                max_response_chars=audit_config.max_response_chars,
                kl_penalty=self.config.algorithm.kl_penalty,
            )
            print(f"Rollout audit reports will be written to {audit_dir}.")
        else:
            self.rollout_auditor = None
        self.global_step = 0
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                # Recompute old-policy log-probs. While the short-lived OPD
                # channel is active, also build detached privileged top-k targets
                # on the exact prefixes sampled by the student.
                with timer("old", timing_raw):
                    opd_weight = 0.0
                    if self.config.algorithm.opd.enabled:
                        opd = self.config.algorithm.opd
                        opd_weight = opd_weight_at_step(
                            step=self.global_step,
                            initial_weight=opd.loss_weight,
                            hold_steps=opd.weight_hold_steps,
                            decay_steps=opd.weight_decay_steps,
                        )
                    if self.config.algorithm.opd.enabled and opd_weight > 0:
                        batch.meta_info.update(
                            {
                                "opd_top_k": opd.top_k,
                                "opd_temperature": opd.temperature,
                                "opd_token_kl_clip": opd.token_kl_clip,
                                "opd_mask_special_tokens": opd.mask_special_tokens,
                            }
                        )
                        opd_targets = self.actor_rollout_ref_wg.compute_opd_targets(batch)
                        batch = batch.union(opd_targets)
                    else:
                ...
                with timer("adv", timing_raw):
                    ...
                    elif self.rollout_auditor is not None:
                        # Online filtering already populated token scores before batch
                        # balancing. Re-score the final ordered batch so audit rows align.
                        _, raw_reward_metrics = ray.get(self.reward_fn.compute_reward.remote(batch))
                    ...
                    if self.config.algorithm.opd.enabled:
                        opd = self.config.algorithm.opd
                        batch.meta_info["opd_temperature"] = opd.temperature
                        batch, opd_metrics = prepare_opd_batch(
                            data=batch,
                            tokenizer=self.tokenizer,
                            loss_weight=opd_weight,
                            token_kl_clip=opd.token_kl_clip,
                            cot_open_tag=opd.cot_open_tag,
                            cot_close_tag=opd.cot_close_tag,
                            allow_legacy_cot=opd.allow_legacy_cot,
                            response_starts_in_cot=opd.response_starts_in_cot,
                            require_nonempty_cot=opd.require_nonempty_cot,
                        )
                ...
                if self.rollout_auditor is not None:
                    if raw_reward_metrics is None:
                        _, raw_reward_metrics = ray.get(self.reward_fn.compute_reward.remote(batch))
                    with timer("audit", timing_raw):
                        audit_metrics = self.rollout_auditor.log_step(
                            step=self.global_step,
                            batch=batch,
        ...
        if self.val_reward_fn is not None and self.config.trainer.validate_at_end:
            if (
                val_metrics is None
        ...
        if self.config.trainer.save_at_end and (
