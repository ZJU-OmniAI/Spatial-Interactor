...
def _swap_visible(obj: VisibleObjectRecord) -> bool:
    ...
    return (
        (obj.pickupable or obj.moveable)
        and obj.object_type not in DISALLOWED_SWAP_TYPES
        and 0.008 <= area <= 0.12
        and obj.distance <= 3.1
        and cx is not None
        and cy is not None
        and 0.10 <= cx <= 0.90
        and 0.10 <= cy <= 0.86
    ...
    def enumerate(self, state: StateRecord) -> List[TaskProposal]:
        ...
        for idx, first in enumerate(ranked):
            for jdx, second in enumerate(ranked):
                ...
                if distance_gap > 0.60:
                    continue
                if cy_gap > 0.24:
                    continue
                if not (0.05 <= cx_gap <= 0.65):
                    continue
                center_bonus = max(0.0, 0.30 - abs(float(first.center_x_ratio or 0.5) - 0.5))
                center_bonus += max(0.0, 0.30 - abs(float(second.center_x_ratio or 0.5) - 0.5))
                score = (
                    float(first.bbox_area_ratio or 0.0)
                    + float(second.bbox_area_ratio or 0.0)
                    + 0.30 * max(0.0, 0.60 - distance_gap)
                    + 0.18 * max(0.0, 0.24 - cy_gap)
                    + 0.18 * cx_gap
                    + 0.12 * center_bonus
                )
                proposals.append(
        ...
        return self._topk(proposals, limit=24)
