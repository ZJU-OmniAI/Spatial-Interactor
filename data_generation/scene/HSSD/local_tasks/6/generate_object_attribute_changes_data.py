...
# -*- coding: utf-8 -*-
"""
HSSD 第6类：改变物体属性 (Object State/Attribute Changes)

逻辑尽量对齐 /path/to/workspace/AI2THOR/6/generate_object_attribute_changes_data.py，
仅把底层执行替换为 Habitat/HSSD。

说明：
1. HSSD scene_instance.json 不包含 AI2THOR 那种 open/toggle 运行时状态字段，
...
class HSSDObjectAttributeChangeGenerator:
    def __init__(
        self,
        output_root: str,
        dataset_root: str,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="HSSD 第6类：改变物体属性 数据生成器")
    parser.add_argument("--output_root", type=str, default="/path/to/workspace/HSSD/6")
...
def main() -> None:
    args = parse_args()
    gen = HSSDObjectAttributeChangeGenerator(
