...
# -*- coding: utf-8 -*-
"""
HSSD 第5类：运动程度比较 (Movement Degree Comparison)
...
class HSSDMovementDegreeRunner(HabitatSceneRunner):
...
class HSSDMovementDegreeComparisonGenerator:
    def __init__(
        self,
        output_root: str,
        dataset_root: str,
        ...
        self.runner = HSSDMovementDegreeRunner(
            ctx=self.ctx,
            image_width=image_width,
            image_height=image_height,
            hfov=hfov,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="HSSD 第5类：运动程度比较 数据生成器")
    parser.add_argument("--output_root", type=str, default="/path/to/workspace/HSSD/5")
...
def main() -> None:
    args = parse_args()
    gen = HSSDMovementDegreeComparisonGenerator(
