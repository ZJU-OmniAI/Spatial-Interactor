...
class HSSDObjectPositionSwappingGenerator:
    def __init__(
        self,
        output_root: str,
        dataset_root: str,
    ...
    def _run_one_sample(self, sample_idx: int, scene_path: str) -> Dict:
        ...
        for obj_a, obj_b, _ in pairs[:MAX_PAIR_ATTEMPTS_PER_VIEW]:
            ...
            current_modified_scene_path = self.ctx.save_temp_scene(modified_scene, scene_path, task_id=7, sample_idx=sample_idx)
            ...
            before_visible = self._visible_object_map(self.runner.unique_visible_objects(scene_objects, start_obs["depth"]))
            after_visible = self._visible_object_map(self.runner.unique_visible_objects(current_scene_objects_after, current_after_obs["depth"]))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="HSSD 第7类：交换两个物体位置 数据生成器")
    parser.add_argument("--output_root", type=str, default="/path/to/workspace/HSSD/7")
...
def main() -> None:
    args = parse_args()
    gen = HSSDObjectPositionSwappingGenerator(
