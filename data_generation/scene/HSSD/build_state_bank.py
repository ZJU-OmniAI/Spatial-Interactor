...
def _prefilter_probe_positions(
    ...
    *,
    ...
    restart_interval = int(getattr(args, "prefilter_restart_interval", 0))
    for chunk_start, position_chunk in _iter_chunks(candidate_positions, restart_interval):
        chunk_runner = runner if chunk_start == 0 else None
        if chunk_runner is None:
            chunk_runner, _, _, _ = _load_scene_with_runner(args, scene)
        print(
            f"[state_bank] scene={scene} step=prefilter_chunk start={chunk_start + 1} "
            f"end={chunk_start + len(position_chunk)} total={len(candidate_positions)}",
            flush=True,
        )
        try:
            ...
            if chunk_runner is not runner:
                _shutdown_runner(chunk_runner)
...
def build_scene_state_bank(args: argparse.Namespace, scene: str) -> dict:
    ...
    try:
        ...
        with state_bank_path.open("w", encoding="utf-8") as fout:
            # HSSD full re-rendering is the unstable phase in Habitat. Keep the
            # stable probe views as final states so proposal generation can finish.
            for group_idx, group in enumerate(selected_groups, start=1):
                if group_idx == 1 or group_idx % 2 == 0:
                    print(
                        f"[state_bank] scene={scene} step=probe_as_final group={group_idx}/{len(selected_groups)}",
                        flush=True,
                    )
                probe_records = list(group["probe_records"])
                probe_records.sort(key=_record_priority, reverse=True)
                picked_records = _select_best_orientations(probe_records, args.max_states_per_position)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build HSSD state bank for fixed scenes.")
