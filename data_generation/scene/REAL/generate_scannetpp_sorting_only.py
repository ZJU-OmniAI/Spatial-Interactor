...
OUTPUT_ROOT = Path("/path/to/workspace/DATA/SCANNETPP_SORTING")
...
def collect_motion_candidates(scenes: list[ScenePose], gaps: Sequence[int]) -> dict[str, list[MotionCandidate]]:
    ...
    for scene in scenes:
        ...
        for label in ACTION_OPTIONS:
            ...
            del per_label[label][80:]
        if all(len(per_label[label]) >= 20 for label in ACTION_OPTIONS):
            break
    return per_label
...
def collect_sorting_rows(scenes: list[ScenePose], target: int, output_root: Path) -> list[dict]:
    ...
    for scene in scenes:
        scene_rows = 0
        for gap in [18, 20, 24, 25, 30, 35, 40, 45, 50, 60]:
            span = 3 * gap
            ...
            for start in range(0, len(scene.frame_ids) - span, max(12, gap // 2)):
                ...
                for motion in motions:
                    ...
                    if not (
                        0.16 <= dz <= 1.25
                        and 0.18 <= translation <= 1.40
                        and abs(dx) <= max(0.18, 0.55 * abs(dz))
                        and abs(dy) <= max(0.25, 0.55 * abs(dz))
                        and max(abs(rx), abs(ry), abs(rz)) <= 13.0
                        and float(motion["abs_rotation_deg"]) <= 16.0
                    ):
                        ok = False
                        break
                    total_forward += dz
                ...
                if loaded is None:
                    continue
                frames, quality = loaded
                if quality["sharpness_min"] < 10 or min(quality["pair_gray_mad"]) < 16:
                    continue
                labels = ["A", "B", "C"]
                ...
                rows.append(
                    {
                        ...
                        "gt": {
                            ...
                            "trajectory_type": "forward_approach_low_rotation",
                            "total_forward_m": float(total_forward),
                            "total_lateral_m": float(total_lateral),
...
def main() -> None:
    ...
    parser.add_argument("--target-per-task", type=int, default=500, help="Maximum rows to generate.")
    ...
    if "action_inference" in selected_tasks:
        gaps = [50, 60, 80, 100, 120, 150, 180, 220]
        per_label = collect_motion_candidates(poses, gaps)
