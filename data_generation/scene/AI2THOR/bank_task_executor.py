...
def _rough_visible_object(
    ...
    *,
    ...
    if quality is None:
        return False
    return (
        quality["area_ratio"] >= 0.004
        and quality["width"] >= 20.0
        and quality["height"] >= 16.0
        and 0.04 <= quality["center_x_ratio"] <= 0.96
        and 0.04 <= quality["center_y_ratio"] <= 0.96
    )
...
class BankTaskExecutor:
    ...
    def _execute_class07(self, controller, state: StateRecord, proposal: TaskProposal) -> dict:
        ...
        if not _rough_visible_object(evt_a, obj_a, width=self.args.width, height=self.args.height):
            raise RuntimeError("class07_object_a_not_visible")
        if not _rough_visible_object(evt_a, obj_b, width=self.args.width, height=self.args.height):
        ...
        along_x = pair_dx / pair_norm if pair_norm > 1e-6 else 0.0
        along_z = pair_dz / pair_norm if pair_norm > 1e-6 else 1.0
        last_error = "class07_teleport_object_failed"
        best_swap = None
        strategies = [
            ("A", object_a_id, object_b_id, pos_a_before, pos_b_before, rot_a_before, rot_b_before),
            ("B", object_b_id, object_a_id, pos_b_before, pos_a_before, rot_b_before, rot_a_before),
        ]
        for lift_label, lifted_id, other_id, lifted_pos_before, other_pos_before, lifted_rot_before, other_rot_before in strategies:
            temp_positions = [
                {
                    "x": float(lifted_pos_before["x"]) + perp_x * side_offset,
                    "y": float(lifted_pos_before["y"]) + lift_height,
                    "z": float(lifted_pos_before["z"]) + perp_z * side_offset,
                },
                {
                    "x": float(lifted_pos_before["x"]) - perp_x * side_offset,
                    "y": float(lifted_pos_before["y"]) + lift_height,
                ...
                {
                    "x": float(lifted_pos_before["x"]) + along_x * 0.10,
                    "y": float(lifted_pos_before["y"]) + lift_height,
                    "z": float(lifted_pos_before["z"]) + along_z * 0.10,
                },
                {
                    "x": float(lifted_pos_before["x"]) - along_x * 0.10,
                    "y": float(lifted_pos_before["y"]) + lift_height,
                ...
                {
                    "x": float(lifted_pos_before["x"]),
                    "y": float(lifted_pos_before["y"]) + lift_height,
                    "z": float(lifted_pos_before["z"]),
                },
            ]
            for temp_pos in temp_positions:
                ...
                if not _rough_visible_object(evt_start, obj_a_start, width=self.args.width, height=self.args.height):
                    last_error = "class07_object_a_not_visible"
                    continue
                if not _rough_visible_object(evt_start, obj_b_start, width=self.args.width, height=self.args.height):
                ...
                if not _rough_visible_object(evt_b, obj_a_after, width=self.args.width, height=self.args.height):
                    last_error = "class07_object_a_after_invisible"
                    continue
                if not _rough_visible_object(evt_b, obj_b_after, width=self.args.width, height=self.args.height):
    ...
    def _class08_build_occlusion_sample(
        ...
        if not movers:
            return None
        candidate_targets = self._class08_candidate_targets(evt_a)
        offsets = list(CLASS08_LOCAL_MOVE_OFFSETS)
        rng.shuffle(offsets)
        prioritized = sorted(
            offsets,
        ...
        for mover in movers[:CLASS08_MAX_CANDIDATE_ATTEMPTS]:
            mover_start = _find_object(evt_a, str(mover["objectId"]))
            same_support_targets = [
                target
                for target in candidate_targets
                if _support_parent_id(target) is not None
                and _support_parent_id(target) == _support_parent_id(mover_start)
                and target.get("objectId") != mover.get("objectId")
            ]
            target_trials = same_support_targets or candidate_targets
            for target in target_trials[:8]:
                evt_seed = self._class08_restore_view(controller, scene_name, start_pose)
                mover_before = _find_object(evt_seed, str(mover["objectId"]))
                target_before = _find_object(evt_seed, str(target["objectId"]))
                if mover_before is None or target_before is None:
                    continue
                if not self._class08_good_target_view(evt_seed, mover_before, relaxed=True):
                    continue
                if not self._class08_good_target_view(evt_seed, target_before, relaxed=True):
                    continue
                target_dx = float(target_before["position"]["x"]) - start_pose.x
                target_dz = float(target_before["position"]["z"]) - start_pose.z
                view_x, view_z = _world_to_local_xz(target_dx, target_dz, start_pose.yaw)
                if abs(view_x) + abs(view_z) < 1e-5:
                    continue
                view_norm = math.hypot(view_x, view_z)
                unit_x = view_x / view_norm
                unit_z = view_z / view_norm
                lateral_x = -unit_z
                lateral_z = unit_x
                mover_size = _aabb_size(mover_before)
                target_size = _aabb_size(target_before)
                clearance = min(
                    0.32,
                    max(
                        0.10,
                        0.5 * max(mover_size["x"], mover_size["z"], 0.08)
                        + 0.5 * max(target_size["x"], target_size["z"], 0.08)
                        - 0.02,
                    ),
                )
                lateral_clearance = min(0.18, max(0.00, 0.5 * min(target_size["x"], target_size["z"], 0.20)))
                targeted_offsets = [
                    (view_x - unit_x * clearance, view_z - unit_z * clearance),
                    (view_x - unit_x * clearance + lateral_x * lateral_clearance, view_z - unit_z * clearance + lateral_z * lateral_clearance),
                    (view_x - unit_x * clearance - lateral_x * lateral_clearance, view_z - unit_z * clearance - lateral_z * lateral_clearance),
                    (view_x - unit_x * (clearance + 0.05), view_z - unit_z * (clearance + 0.05)),
                ]
                for desired_local_x, desired_local_z in targeted_offsets:
                    desired_world_dx, desired_world_dz = _local_to_world_xz(desired_local_x, desired_local_z, start_pose.yaw)
                    target_pos = {
                        "x": start_pose.x + desired_world_dx,
                        "y": float(mover_before["position"]["y"]),
                        "z": start_pose.z + desired_world_dz,
                    }
                    evt_try = self._class08_restore_view(controller, scene_name, start_pose)
                    mover_try = _find_object(evt_try, str(mover["objectId"]))
                    if mover_try is None:
                        continue
                    tele_evt = controller.step(
                        action="TeleportObject",
                        objectId=str(mover["objectId"]),
                        position=target_pos,
                        rotation=dict(mover_try["rotation"]),
                        forceAction=True,
                    )
                    if not bool(tele_evt.metadata.get("lastActionSuccess", False)):
                        continue
                    controller.step(action="AdvancePhysicsStep", simSeconds=0.35)
                    evt_b = controller.last_event
                    mover_after = _find_object(evt_b, str(mover["objectId"]))
                    if not _rough_visible_object(evt_b, mover_after, width=self.args.width, height=self.args.height):
                        continue
                    change = self._class08_best_occlusion_target(evt_try, evt_b, str(mover["objectId"]))
                    if change is None:
                        continue
                    if str(change["target_before"].get("objectId")) != str(target["objectId"]):
                        continue
                    ...
                    move_text = self._class08_move_text(
                        desired_local_x - view_x,
                        desired_local_z - view_z,
                    )
                    return {
                        "event_a": evt_try,
                        "event_b": evt_b,
                        ...
                        "gt": {
                            ...
                            "mover": {
                                "before": self._object_snapshot(evt_try, mover_try),
                                "after": self._object_snapshot(evt_b, mover_after),
                            },
                            "affected_object": {
                                "before": self._object_snapshot(evt_try, change["target_before"]),
                                "after": self._object_snapshot(evt_b, change["target_after"]),
                            },
                            "action_sequence": [
                                {
                                    "api": "TeleportObject",
                                    ...
                                    "rotation": dict(mover_try["rotation"]),
                                    "forceAction": True,
                                    "purpose": "targeted_occlusion_placement",
                                },
                                {"api": "AdvancePhysicsStep", "simSeconds": 0.35, "purpose": "settle_objects"},
                            ],
                            "move_text": move_text,
                            "local_offset": [desired_local_x - view_x, desired_local_z - view_z],
                            "support_parent": {"id": _support_parent_id(mover_try), "type": _support_parent_type(mover_try)},
