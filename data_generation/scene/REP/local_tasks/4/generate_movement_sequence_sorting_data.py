...
# -*- coding: utf-8 -*-
"""
ReplicaCAD 第4类：运动过程排序 (Movement Sequence Sorting) 数据生成器。
...
class ReplicaCADMovementSequenceRunner(HabitatSceneRunner):
...
class ReplicaCADMovementSequenceSortingGenerator:
    def __init__(
        self,
        output_root: str,
        dataset_root: str,
        ...
        self.runner = ReplicaCADMovementSequenceRunner(
            ctx=self.ctx,
            image_width=image_width,
            image_height=image_height,
            hfov=hfov,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ReplicaCAD 第4类：运动过程排序 数据生成器")
    parser.add_argument("--output_root", type=str, default="/path/to/workspace/ReplicaCAD/4", help="输出根目录")
...
def main() -> None:
    ...
    gen = ReplicaCADMovementSequenceSortingGenerator(
