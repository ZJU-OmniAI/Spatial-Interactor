...
# -*- coding: utf-8 -*-
"""
ReplicaCAD 第2类：多图重叠定位 (Multi-image Overlap Localization) 数据生成器。
...
class ReplicaCADOverlapLocalizationRunner(HabitatSceneRunner):
...
class ReplicaCADOverlapLocalizationDataGenerator:
    def __init__(
        self,
        output_root: str,
        dataset_root: str,
        ...
        self.runner = ReplicaCADOverlapLocalizationRunner(
            ctx=self.ctx,
            image_width=image_width,
            image_height=image_height,
            hfov=hfov,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ReplicaCAD 第2类：多图重叠定位 数据生成器")
    parser.add_argument("--output_root", type=str, default="/path/to/workspace/ReplicaCAD/2", help="输出根目录")
...
def main() -> None:
    args = parse_args()
    generator = ReplicaCADOverlapLocalizationDataGenerator(
