...
# -*- coding: utf-8 -*-
"""
ReplicaCAD 第1类：动作逆推 (Action Inference) 数据生成器。
...
class ReplicaCADActionInferenceRunner(HabitatSceneRunner):
...
class ReplicaCADActionInferenceDataGenerator:
    def __init__(
        self,
        output_root: str,
        dataset_root: str,
        ...
        self.runner = ReplicaCADActionInferenceRunner(
            ctx=self.ctx,
            image_width=image_width,
            image_height=image_height,
            hfov=hfov,
            ...
            if desired_pitch < -30.0 or desired_pitch > 60.0:
                for fallback_deg in (45, 30, 15):
                    ...
                    if -30.0 <= candidate_pitch <= 60.0:
                        candidate = self._make_look_action(action.action_api, fallback_deg)
                        action.action_key = candidate.action_key
                        action.action_params = candidate.action_params
                        action.action_natural = candidate.action_natural
                        break
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ReplicaCAD 第1类：动作逆推 数据生成器")
    parser.add_argument("--output_root", type=str, default="/path/to/workspace/ReplicaCAD/1", help="输出目录")
...
def main() -> None:
    ...
    generator = ReplicaCADActionInferenceDataGenerator(
