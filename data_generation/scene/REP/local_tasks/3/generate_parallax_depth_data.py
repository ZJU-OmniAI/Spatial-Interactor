...
# -*- coding: utf-8 -*-
"""
ReplicaCAD 第3类：视差推断深度 (Parallax Depth Inference) 数据生成器。

逻辑对齐 /path/to/workspace/AI2THOR/3/generate_parallax_depth_data.py，
仅将底层接口替换为 Habitat/ReplicaCAD。
"""
...
class ReplicaCADParallaxRunner(HabitatSceneRunner):
...
class ReplicaCADParallaxDepthDataGenerator:
    def __init__(
        self,
        output_root: str,
        dataset_root: str,
        ...
        min_depth_gap: float = 0.8,
        min_obj_dist: float = 0.8,
        max_obj_dist: float = 4.0,
        ...
        self.runner = ReplicaCADParallaxRunner(
            ctx=self.ctx,
            image_width=image_width,
            image_height=image_height,
            hfov=hfov,
    ...
    def _candidate_visible_objects(self, obs: Dict) -> List[object]:
        ...
        for obj in self._all_visible_objects(obs):
            ...
            if counts.get(obj.name, 0) != 1:
                continue
            if not (self.min_obj_dist <= float(obj.distance) <= self.max_obj_dist):
                continue
            if self.runner.target_prominence_score(obj) < 2.0:
                continue
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ReplicaCAD 第3类：视差推断深度 数据生成器")
    parser.add_argument("--output_root", type=str, default="/path/to/workspace/ReplicaCAD/3", help="输出根目录")
...
def main() -> None:
    args = parse_args()
    generator = ReplicaCADParallaxDepthDataGenerator(
