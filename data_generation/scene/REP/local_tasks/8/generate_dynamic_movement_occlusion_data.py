...
# -*- coding: utf-8 -*-
"""
ReplicaCAD 第8类：动态移动与遮挡 (Dynamic Movement / Occlusion)

逻辑尽量对齐 /path/to/workspace/AI2THOR/8/generate_dynamic_movement_occlusion_data.py，
并参考 /path/to/workspace/HSSD/8/generate_dynamic_movement_occlusion_data.py，
仅把底层执行替换为 Habitat/ReplicaCAD 的 scene_instance.json 改写。

说明：
1. 保留 AI2THOR 中 distance_change / occlusion_change 两个题族和同样的调度顺序。
2. 视角固定为正常人高度 + 平视，并额外过滤两侧墙面过多、目标不显眼的起始视角。
3. ReplicaCAD 不提供 AI2THOR 的 2D instance bbox，这里改用“投影中心 + 深度前后关系 + 显著性变化”
...
class ReplicaCADDynamicMovementOcclusionGenerator:
    def __init__(
        self,
        output_root: str,
        dataset_root: str,
    ...
    def _screen_center_distance_ratio(a, b, image_width: int, image_height: int) -> float:
        return ReplicaCADDynamicMovementOcclusionGenerator._screen_center_distance(a, b) / float(
    ...
    def _best_occlusion_target(self, scene_objects: Sequence[Dict], attempt: Dict) -> Optional[Dict]:
        before_visible_unique = self._candidate_targets(
            scene_objects, attempt["event_a"], exclude_indexes={int(attempt["mover_before"].index)}
        )
        after_visible_all = self._visible_object_map_by_index(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ReplicaCAD 第8类：动态移动与遮挡 数据生成器")
    parser.add_argument("--output_root", type=str, default="/path/to/workspace/ReplicaCAD/8")
...
def main() -> None:
    args = parse_args()
    gen = ReplicaCADDynamicMovementOcclusionGenerator(
